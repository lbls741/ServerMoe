...
> 状态：**M0–M4 已完成**（M2 真机验收；初始提交 + M4 提交已入库）。M4 交付：完整管理页（账号状态/sendkey 轮换/绑定向导/关键词管理/未命中提醒设置/收发日志），配套 admin API 与测试。60 测试全绿，镜像 m4 冒烟通过。下一步：M5 邮件桥（可选）→ M6 多用户联调 → M7 加固交付。
